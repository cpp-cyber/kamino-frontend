...
export async function handleAdminPodDeployment(
  ...
  // Show immediate feedback
  toast.info(`Deploying "${template}" to ${targetCount} target${targetCount === 1 ? '' : 's'}...`, {
  ...
  try {
    ...
    toast.success(`"${template}" deployed to ${targetCount} target${targetCount === 1 ? '' : 's'}`)
...
export async function handleCreateUsers(
  ...
  // Show immediate feedback
  toast.info(`Creating ${isSingle ? `user "${users[0].username}"` : `${userCount} users`}...`, {
    duration: 10000,
  })
  ...
  try {
    ...
    const createError = error instanceof Error ? error : new Error('Unknown creation error')
    
    toast.error(isSingle 
...
export async function handleDeleteUsers(
  ...
  // Show immediate feedback
  toast.info(`Deleting ${isSingle ? `user "${usernames[0]}"` : `${userCount} users`}...`, {
    duration: 10000,
  })
  ...
  try {
    ...
    const deleteError = error instanceof Error ? error : new Error('Unknown deletion error')
...
/**
 * Handles user enable/disable operations with immediate feedback
 */
export async function handleToggleUsers(
  usernames: string[],
  ...
  const userCount = usernames.length
  const isSingle = userCount === 1
  const actionVerb = action === 'enable' ? 'Enabling' : 'Disabling'
  const actionPast = action === 'enable' ? 'enabled' : 'disabled'
  
  // Show immediate feedback
  toast.info(`${actionVerb} ${isSingle ? `user "${usernames[0]}"` : `${userCount} users`}...`, {
    duration: 8000,
  })

  try {
    if (action === 'enable') {
      await enableUsers(usernames)
    } else {
      await disableUsers(usernames)
    }
    console.log(`Successfully ${actionPast} ${userCount} user(s)`)
    
    toast.success(isSingle 
      ? `User "${usernames[0]}" ${actionPast} successfully`
      : `${userCount} users ${actionPast} successfully`
    )
    ...
    const toggleError = error instanceof Error ? error : new Error(`Unknown ${action} error`)
    
    toast.error(isSingle 
      ? `Failed to ${action} user "${usernames[0]}": ${toggleError.message}`
      : `Failed to ${action} ${userCount} users: ${toggleError.message}`
    )
...
export async function handleUserGroupOperation(
  ...
  // Show immediate feedback
  toast.info(`${actionVerb} ${isSingle ? `user "${usernames[0]}"` : `${userCount} users`} ${preposition} group "${groupName}"...`, {
    duration: 8000,
  })
  ...
  try {
    ...
    console.log(`Successfully ${actionPast} group "${groupName}": ${userCount} user(s)`)
    ...
    const groupError = error instanceof Error ? error : new Error(`Unknown group ${action} error`)
...
export async function handleCreateGroups(
  ...
  // Show immediate feedback
  toast.info(`Creating ${isSingle ? `group "${groupNames[0]}"` : `${groupCount} groups`}...`, {
    duration: 8000,
  })
...
export async function handleDeleteGroups(
  ...
  // Show immediate feedback
  toast.info(`Deleting ${isSingle ? `group "${groupNames[0]}"` : `${groupCount} groups`}...`, {
    duration: 8000,
  })
...
export async function handleRenameGroup(
  ...
  try {
    ...
    const renameError = error instanceof Error ? error : new Error('Unknown rename error')
