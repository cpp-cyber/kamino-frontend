...
import { Eye, Boxes, Rocket, User, Copy } from "lucide-react";
...
export default function Page() {
  const { isDialogOpen, selectedPod, openDeployDialog, closeDeployDialog } =
    usePodDeployment();
                        ...
                        <Tooltip>
                          <TooltipTrigger>
                            <span className="font-semibold text-foreground">
                              <Badge
                                variant="secondary"
                                className="bg-card shadow-md dark:shadow-primary/10"
                              >
                                {deployedPods?.length || 0} / 5
                              </Badge>
                            </span>
                          </TooltipTrigger>
          ...
          <Card className="@container/card bg-gradient-to-br from-primary/5 to-kamino-green/5 border-primary/20">
            <CardHeader>
              <CardTitle>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Library of templates created by students and faculty,
                  available for users to clone and interact with.
                </p>
              </CardTitle>
              <CardAction className="text-xs">
                <Button variant="link" size="sm" className="mt-2">
                  <Eye />
                  <Link href="/pods/templates" passHref>
                    View All
                  </Link>
                </Button>
              </CardAction>
                            ...
                            <div className="flex items-center gap-4 mb-4">
                              <div className="bg-sidebar-accent-foreground/10 text-sidebar-primary-foreground flex size-16 items-center justify-center rounded-lg overflow-hidden shadow-lg shrink-0">
                                <Image
                                  src={
                                    template.image_path
                                      ? `/api/v1/template/image/${template.image_path}`
                                      : "/kaminoLogo.svg"
                                  }
                                  alt={template.name}
                                  width={64}
